...
写条件时，可以多问一句“读代码的人会不会立刻知道我在判断什么”。当条件开始同时承担空值检查、状态判断和业务规则时，优先拆出具名布尔变量，通常比追求一行写完更稳妥。

`if/else` 和 `switch` 也不是谁更高级，而是谁更适合当前问题：连续范围判断更适合 `if/else`，离散状态分发更适合 `switch`。
...
条件控制的难点从来不只是语法，而是把分支意图写清楚。条件越清楚，后续重构、调试和排查问题就越轻松；这也是现代 `C++` 在条件表达式上最强调的习惯。
