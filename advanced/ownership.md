...
这篇文章的目标，就是把所有权和 `RAII` 这条主线讲清楚。
默认你已经知道构造函数、析构函数、拷贝和移动这些语法名字；
如果你还停留在“类怎么定义、对象怎么创建”这一层，建议先看前面的 `类和对象`。
...
某个对象负责资源的生命周期，资源何时释放由它决定。

## 2. 借用

某个对象或函数只是临时使用资源，但不负责释放。

## 3. 观察

某个对象只保留一个“能找到它”的引用或指针，用来访问，但不拥有。
...
`RAII` 是 `Resource Acquisition Is Initialization` 的缩写，通常翻成：
...
> 优先让对象自己管理资源，而不是在业务流程里散落清理逻辑。

----

# 资源类型

很多初学者会把“资源”狭义地理解成内存，但工程里资源远不止这个。
