...
这叫函数重载。它的意义不是“省名字”，而是让一组语义相近的操作拥有统一的接口名。
...
这样调用者看到的都是 `print(...)`，但编译器会根据参数类型选择合适的版本。

需要注意的是，重载解析并不是“名字一样就行”，编译器还会考虑：
...
// f('a');  // 会参与重载匹配，阅读时要注意编译器如何选最优匹配
```

> 重载很方便，但一旦配合隐式转换和默认参数，接口就可能变得不够直观。
