...
`struct` 这一章真正有用的，不是区分它和 `class` 的语法细节，而是学会把它当成一种意图信号：这个类型主要是轻量数据载体，还是已经开始承载复杂封装规则。读代码时，这个判断会非常省力。
...
现代 `C++` 里的 `struct` 不是低配版 `class`，而是更适合轻量数据模型的一种默认表达。只要把“数据载体”和“强封装对象”区分开，`struct` 和 `class` 的使用边界就会清楚很多。
