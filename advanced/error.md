...
真实项目里，很多团队会根据场景混合使用 `返回值` 和 `异常`，而不是只用一种方式。
...
`C++` 异常最核心的价值，不只是“跳到别处处理错误”，而是 **栈展开**
...
如果 `run()` 抛出异常，`b` 和 `a` 的析构函数仍会被调用。

> 这就是为什么 `RAII` 和异常安全在 `C++` 里总是绑在一起讨论。
...
doSomething();
// 那么异常发生时，`file` 离开作用域会自动析构，资源也就自动释放
// 也正因此，现代 `C++` 鼓励尽量把资源管理放进对象，而不是散落在流程代码里
```

## 标准异常类型

> 标准库里常见的异常基类是：`std::exception`
...
`C++11` 以 `noexcept` 作为 **异常说明**：标明函数是否可能抛出异常，供调用方与标准库实现作契约推断。
...
- 语义上属于 **接口契约**，与“优化提示”不可等同视之。
- **析构函数** 隐含 `noexcept(true)` 倾向；析构中抛异常在异常传播期间具有极高风险，故资源清理常采用不抛契约或吞并记录策略。
- **移动成员函数** 若声明为 `noexcept`，标准容器在重新分配等操作中更易选用移动而非拷贝；若移动可能抛异常，部分容器会退回复制路径，影响性能与安全边界。

读代码时，将 `noexcept` 理解为 **对异常传播范围的显式约束** 即可；细则可结合具体类型的移动、析构与容器接口查阅标准库要求。

# 异常与析构函数

> 一个非常重要的经验是：`析构函数通常不应该抛出异常。`
