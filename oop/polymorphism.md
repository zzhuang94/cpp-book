...
对初学者来说，多态最初往往被理解成“基类指针指向派生类对象”。但在真实项目里，更关键的问题通常是：
...
也就是说，多态的重点是设计扩展点，而不只是语法演示。

## 为什么 `override` 和虚析构这么重要

这两个点看起来都不大，却几乎决定了多态代码是否可靠：

- `override` 帮你确认确实覆盖成功
- 虚析构确保通过基类指针销毁时行为正确

如果你读一个多态接口，却没看到虚析构和 `override`，就应该提高警惕。
...
如果它被设计为多态基类，通常仍应提供虚析构。

### 误区 3：只要写了虚函数，多态就稳了

不对。传值切片、错误销毁、签名不匹配都可能让设计失效。

## 补充建议

- 多态边界优先设计得窄而清晰。
- 多态接口传递优先使用引用、指针或智能指针。
- 重写虚函数始终加 `override`。
- 只有真的需要运行时扩展点时才引入多态。
