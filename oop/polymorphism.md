...
基类指针或引用实际指向派生类对象，且需在 **静态类型已为基类** 的前提下使用派生类专有成员时，可使用 `dynamic_cast`。  
对指针目标类型，失败时得到空指针；对引用目标类型，失败时抛出 `std::bad_cast`。  
适用对象须为多态类型：一般要求基类存在虚函数表，常见做法是提供 **虚析构函数**。

`typeid` 作用于多态类型的左值时，可反映对象的 **动态类型** 所对应的 `std::type_info`；用于非多态类型或部分表达式时，语义以标准条款为准，工程中不宜将其比较结果作为核心业务分支的唯一依据。

部分翻译单元以 **关闭 RTTI** 的选项构建（例如 `-fno-rtti`）。在此类配置下，`dynamic_cast` 与 `typeid` 的可用性受限，须在构建说明或接手材料中先行核对。
