...
所以读类时，不能只看字段和函数，还要看它如何被构造、复制、移动和销毁。

## 值语义和引用语义要在类这一章真正建立起来
...
这和很多默认对象引用语义更强的语言不太一样。也正因此，`C++` 类设计常常需要明确回答：
...
初学阶段不必把所有术语背得非常死，但至少要形成一个直觉：

- 如果类不自己管理特殊资源，尽量依赖默认生成行为
- 如果类自己管理资源，就必须认真思考拷贝、移动和析构行为

这就是很多现代 `C++` 讨论里常说的“规则零/三/五”的背景。

## 初始化列表不是形式，而是更合理的默认方式

对成员对象来说，初始化列表通常比“先默认构造再赋值”更直接，也更高效。因此在阅读和编写类时，初始化列表应该尽快成为默认直觉。
...
### 误区 2：能复制的类总比不能复制的类更方便

不一定。对资源句柄类来说，禁止复制往往更安全。
